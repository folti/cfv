...
if has_sha224 is True:
    fmt_info['sha224'] = (1, 0, 1, 1)

if has_sha256 is True:
    fmt_info['sha256'] = (1, 0, 1, 1)

if has_sha512 is True:
    fmt_info['sha512'] = (1, 0, 1, 1)
...
rx_End = r'(, \d+ differing cases)?(, \d+ quoted filenames)?(, \d+ tested in textmode)?.  [\d.]+ seconds, [\d.]+K(/s)?$'
...
def T_test(f, extra=None):
    ...
    test_generic(cmd + " -v -T --list0=unverified -f test" + f + " test.py testfix.csv data1", cfv_listdata_unv_test,
                 stderr="/dev/null")
...
def C_funkynames_test(t):
    ...
    try:
        ...
        for i in range(1, 256):
            ...
            if t in ('sfv',
                     'sfvmd5') and n == ';': n = 'foo' + n  # ';' is comment character in sfv files, filename cannot start with it.
...
def all_tests():
    ...
    for t in 'sha1', 'sha256', 'sha224', 'sha512', 'md5', 'bsdmd5', 'sfv', 'sfvmd5', 'csv', 'csv2', 'csv4', 'crc', 'par', 'par2':
        search_test(t)
    ...
    hashtests = ['sha1', 'md5', 'bsdmd5', 'sfvmd5']
    if has_sha224 is True:
        hashtests += ['sha224', ]
    if has_sha256 is True:
        hashtests += ['sha256', ]
    ...
    crctests = ['csv', 'csv2', 'csv4', 'sfv', 'crc']

    for htest in hashtests + crctests:
        T_test('.%s' % htest)
    ...
    C_test("bsdmd5", "-t bsdmd5")  # bsd md5 seems to have no way to check, only create

    def C_test_external_verify(type, sumcmd):
        verfycmd = None
        fcmd = "%s -c" % sumcmd
        if pathfind(sumcmd):# don't report pointless errors on systems that don't have sha1sum
            verifycmd = lambda f: test_external(fcmd + f, status_test)
        C_test(type, verify=verfycmd)

    C_test_external_verify('sfv', 'cksfv')
    C_test_external_verify('md5', 'md5sum')
    C_test_external_verify('sha1', 'sha1sum')
    C_test_external_verify('sha224', 'sha224sum')
    C_test_external_verify('sha256', 'sha256sum')
    C_test_external_verify('sha512', 'sha512sum')
    ...
    test_generic(cfvcmd + " -m -v -T -t sha224", lambda s, o: cfv_typerestrict_test(s, o, 'sha224'))
    test_generic(cfvcmd + " -m -v -T -t sha256", lambda s, o: cfv_typerestrict_test(s, o, 'sha256'))
    test_generic(cfvcmd + " -m -v -T -t sha512", lambda s, o: cfv_typerestrict_test(s, o, 'sha512'))
    ...
    if BitTorrent:
        ...
        for strip in (0, 1):
            ...
            test_generic(
                cfvcmd + " -T --strippaths=%s -p foo2err -f %s" % (strip, os.path.join(os.pardir, "foo.torrent")),
            ...
            test_generic(
                cfvcmd + " -T --strippaths=%s -p foo2err1 -f %s" % (strip, os.path.join(os.pardir, "foo.torrent")),
